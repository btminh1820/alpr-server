...
from Models_Architecture_Source.utils.google_utils import gsutil_getsize
from Models_Architecture_Source.utils.metrics import fitness
from Models_Architecture_Source.utils.torch_utils import init_torch_seeds
...
def polygon_inter_union_cpu(boxes1, boxes2):
    ...
    for i in range(n):
        boxes1_ = boxes1[i, :].view(4,2)
        polygon1 = shapely.geometry.Polygon([(boxes1_[k, 0], boxes1_[k, 1]) for k in range(len(boxes1_))]).convex_hull
        for j in range(m):
            boxes2_ = boxes2[j, :].view(4,2)
            polygon2 = shapely.geometry.Polygon([(boxes2_[l, 0], boxes2_[l, 1]) for l in range(len(boxes2_))]).convex_hull
