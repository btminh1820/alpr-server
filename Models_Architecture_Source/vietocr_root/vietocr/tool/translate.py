...
from vietocr_root.vietocr.model.transformerocr import VietOCR
from vietocr_root.vietocr.model.vocab import Vocab
from vietocr_root.vietocr.model.beam import Beam


def vis_feature_maps(feature_map_stack):
    processed = []
    for ft_map in feature_map_stack:
        ft_map = ft_map.squeeze(0)
        gray_scale = torch.sum(ft_map,0)
        gray_scale = gray_scale / ft_map.shape[0]
    ...
    plt.savefig(str(r'D:\license_plate_rec\code\vietocr_root\image\encoder_feature_maps.jpg'), bbox_inches='tight')     
...
def translate_beam_search(img, model, beam_size=4, candidates=1, max_seq_length=128, sos_token=1, eos_token=2):
    ...
    with torch.no_grad():
        src, src_2d = model.cnn(img)
        #vis_feature_maps(src_2d)
